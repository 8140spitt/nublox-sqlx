...
function diffSnapshots(a, b) {
    ...
    for (const t of b.tables)
        if (!aKeys.has(`${t.schema}.${t.name}`))
            changes.push({ kind: 'createTable', table: t });
    for (const t of a.tables)
        if (!bKeys.has(`${t.schema}.${t.name}`))
            changes.push({ kind: 'dropTable', ref: { schema: t.schema, name: t.name } });
    const phases = ['tables:create', 'tables:alter', 'indexes', 'fks', 'views', 'cleanup'];
    const planHash = (0, hash_1.sha256)(JSON.stringify(changes));
