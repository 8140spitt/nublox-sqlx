...
import type {
    SqlxDriver, QueryResult, Isolation, ClientEvents, EventName
} from './types';
...
export type ClientConfig = {
    ...
    // Timeouts & retries
    defaultQueryTimeoutMs?: number; // default 0 = no timeout
    maxRetries?: number;            // default 2
    deadlockRetry?: boolean;        // default true
...
export class SqlxClient {
    ...
    // Event helpers
    on<K extends EventName>(name: K, fn: (e: ClientEvents[K]) => void) { return this.emitter.on(name, fn); }
    onAny(fn: (name: EventName, payload: any) => void) {
        const names: EventName[] = ['connect', 'disconnect', 'error', 'retry', 'reconnect', 'query:start', 'query:end'];
        const offs = names.map(n => this.on(n as any, (e: any) => fn(n, e)));
        return () => offs.forEach(off => off());
    }

    get isHealthy() { return this.healthy; }
    get threadId() { try { return this.drv?.threadId?.(); } catch { return undefined; } }
    ...
    async tx<T>(fn: () => Promise<T>, opts?: { isolation?: Isolation }) {
     ...
     */
    async exec(sql: string, params?: unknown[], opt?: {
        timeoutMs?: number;
        maxRetries?: number;
                ...
                const guarded = (timeoutMs && timeoutMs > 0)
                    ? new Promise<QueryResult>((resolve, reject) => {
                        timeoutHandle = setTimeout(async () => {
                            timedOut = true;
                            try { await this.drv.kill?.(); } catch { /* ignore */ }
                            reject(new Error(`SQLxClient timeout after ${timeoutMs}ms`));
                        }, timeoutMs);
                        p.then((v) => { clearTimeout(timeoutHandle); resolve(v); },
                            (e) => { clearTimeout(timeoutHandle); reject(e); });
                    })
                    : p;
    ...
    async prepare(_sql: string) { throw new Error('prepare() not implemented in client; use driver-specific prepare'); }
