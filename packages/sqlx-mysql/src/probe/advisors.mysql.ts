// Loose-typed advisors to fit current CIR shapes without fighting TS definitions.
...
function idxCols(idx: any): string[] {
    // CIR_Index typically has parts: [{ colOrExpr, prefixLen }]
    const parts: any[] = Array.isArray(idx?.parts) ? idx.parts : [];
    return parts.map((p) => String(p?.colOrExpr ?? '')).filter(Boolean);
}

export function adviseMySQL(snapshot: any): AdvisorFinding[] {
    ...
    for (const t of tables) {
        ...
        const indexes: any[] = Array.isArray(t?.indexes) ? t.indexes : [];
        const fks: any[] = Array.isArray(t?.foreignKeys) ? t.foreignKeys
        ...
        for (const fk of fks) {
            const childCols: string[] = Array.isArray(fk?.columns) ? fk.columns : [];
