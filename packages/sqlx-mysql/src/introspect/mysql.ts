import type {
    ...
    CIR_FK,
    CIR_View
} from "@nublox/sqlx-core";
import { sha256 } from "@nublox/sqlx-core";

function normalizeIndexType(mysqlType: string | null):
    "btree" | "hash" | "fulltext" | "spatial" | "gist" | "gin" | "brin" | "unknown" | undefined {
    if (!mysqlType) return undefined;
...
export async function introspectMySQL(
    ...
    const schemasRes = await exec(
        `SELECT SCHEMA_NAME AS name
       FROM information_schema.SCHEMATA
      WHERE SCHEMA_NAME NOT IN ('mysql','information_schema','performance_schema','sys')
      ORDER BY 1`
    );
    const schemas = schemasRes.rows.map((r: any) => r.name);

    const tables: CIR_Table[] = [];
    const views: CIR_View[] = [];

    for (const schema of schemas) {
        const tRes = await exec(
        ...
        for (const t of tRes.rows) {
          ...
          ORDER BY ORDINAL_POSITION`,
            ...
            const columns: CIR_Column[] = cRes.rows.map((c: any) => ({
                name: c.COLUMN_NAME,
                type: c.COLUMN_TYPE,
          ...
          ORDER BY INDEX_NAME, SEQ_IN_INDEX`,
            ...
            const indexes: CIR_Index[] = Array.from(indexMap.entries()).map(([name, parts]) => ({
                name,
          ...
          ORDER BY k.ORDINAL_POSITION`,
            ...
            const fks: CIR_FK[] = Array.from(fkMap.entries()).map(([name, parts]) => ({
                name,
