export class Reader {
    ...
    lenenc() {
        ...
        if (b === 0xfd) {
            const v = this.buf.readUIntLE(this.off, 3);
            this.off += 3;
            return v;
        }
    ...
    str(len) { const s = this.buf.toString('utf8', this.off, this.off + len); this.off += len; return s; }
    nulstr() { const end = this.buf.indexOf(0x00, this.off); const s = this.buf.toString('utf8', this.off, end); this.off = end + 1; return s; }
    slice(n) { const s = this.buf.subarray(this.off, this.off + n); this.off += n; return s; }
