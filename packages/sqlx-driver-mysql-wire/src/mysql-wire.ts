...
// Local copy of the minimal driver shape (keeps this package standalone)
export interface QueryResult { rows: any[]; affectedRows?: number; insertId?: number | bigint; }

function scrambleNativePassword(password: string, nonce: Buffer) {
    const sha1 = (d: Buffer | string) => createHash('sha1').update(d).digest();
...
export class MysqlWire {
    ...
    private authPlugin = 'mysql_native_password';
    ...
    async connect(url: string) {
        ...
        r.u8(); r.nulstr();             // protocol, serverVersion
        this.threadId = r.u32();
        const scramble1 = r.slice(8); r.u8();
        r.u16(); r.u8(); r.u16();       // cap1, charset, status
        const cap2lo = r.u16(); const cap2hi = r.u16();
        ...
        if (r.remaining > 0) this.authPlugin = r.nulstr() || this.authPlugin;
        ...
        if (auth[0] === 0xff) {
            const er = new Reader(auth); er.u8(); const code = er.u16();
    ...
    async kill() {
        if (this.threadId) await this.query(`KILL QUERY ${this.threadId}`);
    }

    async query(sql: string): Promise<QueryResult> {
        const w = new Writer(); w.u8(0x03).str(sql); // COM_QUERY
        await this.writePacket(w.build());
        return this.readResultset();
    }

    private async command(cmd: number) {
        const w = new Writer(); w.u8(cmd); await this.writePacket(w.build());
    }

    private async readResultset(): Promise<QueryResult> {
        ...
        for (let i = 0; i < colCount; i++) await this.readPacket();
    ...
    private async writePacket(payload: Buffer) {
    ...
    private read(n: number): Promise<Buffer> {
        return new Promise((resolve, reject) => {
            ...
            const onErr = (e: any) => { this.sock.off('error', onErr); this.sock.off('data', onData); reject(e); };
